...
function ProjectCard({ project, index, onView, isTouch }) {
  ...
  const overlayShown = isTouch ? tapped : undefined;

  return (
    <article
      className="project-card relative shrink-0 w-[78vw] sm:w-[55vw] lg:w-[44vw] xl:w-[40vw] group"
      ...
      <div className="relative w-full aspect-[16/10] overflow-hidden rounded-[2px] bg-graphite/5 border border-white/[0.06] transition-[border-color] duration-700 group-hover:border-accent/30">
        ...
        <div
          className={`
            ...
            flex flex-col justify-end p-5 lg:p-8
            transition-opacity duration-500
            ${overlayShown === true ? "opacity-100"
              : overlayShown === false ? "opacity-0"
              : "opacity-0 group-hover:opacity-100"}
          `}
        >
          <div
            className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
            style={overlayShown ? { transform: "translateY(0)" } : undefined}
          >
            ...
            <h3 className="font-display text-xl sm:text-2xl lg:text-4xl text-white tracking-[-0.02em] leading-none mb-3">
              {project.title}
            </h3>
            {project.description && (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="inline-flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.3em] px-3 py-2 rounded-[2px] bg-accent text-void border border-accent transition-all duration-300 hover:bg-transparent hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"
              >
                <IconEye />
                View
              </button>
...
function EndCard() {
  return (
    <div
      className="shrink-0 self-center h-[44vw] sm:h-[38vh] lg:h-[46vh] aspect-[3/4] flex flex-col items-center justify-center text-center p-5 sm:p-8 lg:p-10 rounded-[2px] ml-2 lg:ml-8 bg-[#0a0a0a] border border-[#1a1a1a] relative overflow-hidden"
      aria-hidden="true"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-accent/[0.06] via-transparent to-transparent pointer-events-none" />
      <span className="absolute top-3 left-3 w-3 h-3 border-t border-l border-accent/20" />
      <span className="absolute bottom-3 right-3 w-3 h-3 border-b border-r border-accent/20" />
      <p className="font-mono text-[7px] uppercase tracking-[0.5em] text-accent/60 mb-4 relative z-10">Portfolio Archive</p>
      <h3 className="font-display text-xl sm:text-2xl lg:text-3xl text-[#f5f5f7] leading-snug tracking-[-0.02em] mb-6 relative z-10">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="relative z-10 inline-flex items-center gap-2 font-mono text-[7px] uppercase tracking-[0.4em] px-4 py-2 rounded-[2px] border border-accent/30 text-accent hover:bg-accent hover:text-void transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"
      >
        <IconFolder />
        View Drive
      </a>
...
export function ProjectGrid() {
  ...
  /* ── GSAP pin — desktop non-touch only ─────────────────────────────────── */
  useEffect(() => {
  ...
  /* ── shared card track ─────────────────────────────────────────────────── */
  const cardTrack = loading ? (
    [0, 1, 2].map(i => (
      <div key={i} className="w-[78vw] sm:w-[55vw] lg:w-[44vw] aspect-[16/10] bg-graphite/20 animate-pulse rounded-[2px] shrink-0" />
    ))
  ) : (
    <>
      {projects.map((p, idx) => (
  ...
  /* ── MOBILE / TOUCH — native horizontal swipe, same cards ─────────────── */
  if (isTouch || reduce) {
    return (
      <>
        <section
        ...
        >
          ...
          <div className="px-5 sm:px-8 mb-7">
            <p className="font-mono text-[9px] uppercase tracking-[0.5em] text-accent mb-2.5">Selected Works</p>
            <div className="flex items-end justify-between gap-4">
              <h2 className="font-display text-4xl sm:text-5xl text-bone tracking-[-0.03em] leading-none">
                Featured<br />
                <span className="italic pl-7 sm:pl-10 text-accent">Editorials</span>
              </h2>
              <a
                href={DRIVE_ALL_LINK}
                target="_blank"
                rel="noopener noreferrer"
                className="shrink-0 inline-flex items-center gap-1.5 font-mono text-[7px] uppercase tracking-[0.3em] px-3 py-2 rounded-[2px] border border-accent/25 text-accent/70 hover:border-accent hover:text-accent transition-all duration-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent/50 mb-1"
              >
                <IconFolder />
                Drive
              </a>
            </div>
            <p className="mt-3 font-sans text-sm text-mist/50 leading-relaxed max-w-xs border-l border-white/[0.06] pl-4">
              A curated selection across fashion, architecture, and lifestyle.
            </p>
          </div>

          {/* native swipe track — same cards as desktop */}
          <div
            className="h-scroll-fallback flex gap-5 sm:gap-7 px-5 sm:px-8 pb-2 items-end"
            role="list"
    ...
    <>
      ...
      >
        <div className="sticky top-0 h-screen-d flex flex-col justify-center overflow-hidden">
          ...
          <div
            ref={horizontalRef}
            className="flex gap-6 lg:gap-16 px-6 lg:px-20 items-end will-change-transform shrink-0"
